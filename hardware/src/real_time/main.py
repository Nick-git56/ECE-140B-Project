...
class Environment:
    ...
    def simulation(self):
        ...
        # #ending clause
        # print("Main program completed. Scan NFC tag to confirm shutoff.")
...
if __name__ == "__main__":
    ...
    #add handles for the threads
    # simulation_thread = Thread(target=env.simulation)#,args=(env.scanner,))
    # scanner_thread = Thread(target=env.scanner.scan)

    #start simulation, scanner waits for shutoff signal from RFID object
    # env.run(simulation=simulation_thread)#, scanner = scanner_thread)
    env.simulation()
